...
const SmartNotesView = lazy(() =>
  import('./features/smartNotes/SmartNotesView').then((module) => ({ default: module.SmartNotesView })),
);
...
const SystemHealthView = lazy(() =>
  import('./features/systemHealth/SystemHealthView').then((module) => ({ default: module.SystemHealthView })),
);
const IStokView = lazy(() =>
  import('./features/istok/IStokView').then((module) => ({ default: module.IStokView })),
);
const TeleponanView = lazy(() =>
  import('./features/teleponan/TeleponanView').then((module) => ({ default: module.TeleponanView })),
);
      ...
      <Suspense fallback={<ViewLoader />}>
        <div
          className={`h-full w-full transition-opacity duration-300 ${
            isPending ? 'opacity-70 scale-[0.99] origin-center' : 'opacity-100 scale-100'
          }`}
        >
          {(() => {
              ...
              case 'dashboard':
                return (
                  <ErrorBoundary viewName="DASHBOARD">
                    <DashboardView key={language} onNavigate={handleNavigate} notes={notes} />
                  </ErrorBoundary>
                );
              case 'notes':
                return (
                  <ErrorBoundary viewName="ARCHIVE_VAULT">
                    <SmartNotesView key={language} notes={notes} setNotes={setNotes} />
                  </ErrorBoundary>
                );
              case 'chat':
                return (
                  <ErrorBoundary viewName="NEURAL_LINK">
                    <AIChatView key={language} chatLogic={chatLogic} />
                  </ErrorBoundary>
                );
              case 'tools':
                return (
                  <ErrorBoundary viewName="NEURAL_ARSENAL">
                    <AIToolsView key={language} />
                  </ErrorBoundary>
                );
              case 'system':
                return (
                  <ErrorBoundary viewName="SYSTEM_HEALTH">
                    <SystemHealthView key={language} />
                  </ErrorBoundary>
                );
              case 'settings':
                return (
                  <ErrorBoundary viewName="CORE_CONFIG">
                    <SettingsView key={language} onNavigate={handleNavigate} />
                  </ErrorBoundary>
                );
              default:
                return (
        ...
        <div className="flex w-full h-full bg-skin-surface/80 border border-skin-border rounded-[32px] shadow-sm overflow-hidden">
          <div className="flex w-full h-full bg-skin-card/40">
            ...
            <main className="flex-1 relative h-full w-full bg-transparent min-w-0 flex flex-col">
              <div className="flex-1 w-full h-full overflow-hidden relative">{renderContent()}</div>
            </main>
          </div>
        </div>
      </div>

      <MobileNav activeFeature={activeFeature} setActiveFeature={handleNavigate} chatLogic={chatLogic} />

      <LiveMiniPlayer />
      <DebugConsole isOpen={isDebugOpen} onClose={() => setIsDebugOpen(false)} />

      {!isTutorialComplete && (
        <TutorialOverlay onComplete={() => setIsTutorialComplete(true)} onNavigate={handleNavigate} />
      )}
    </div>
  );
};
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    if (incomingConnection) {
      ...
      const process = async () => {
        ...
        const payload =
          (await decryptData(firstData.payload, '000000')) || (await decryptData(firstData.payload, '123456'));
        if (payload) {
          try {
            const json = JSON.parse(payload);
            setRequestIdentity(json.identity || 'Unknown Agent');
          } catch (e) {
            setRequestIdentity('Encrypted Signal');
          }
        } else {
  ...
  if (sessionMode === 'ISTOK') {
    return (
      <Suspense
        fallback={<div className="h-screen w-screen bg-black flex items-center justify-center text-red-500 font-mono">INITIALIZING_SECURE_LAYER...</div>}
      >
  ...
  if (sessionMode === 'TELEPONAN') {
    return (
      <Suspense
        fallback={<div className="h-screen w-screen bg-black flex items-center justify-center text-green-500 font-mono">INITIALIZING_SECURE_VOICE...</div>}
      >
